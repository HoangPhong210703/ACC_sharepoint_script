"""Ingest 3 SharePoint Excel files into 9 Postgres tables.

Applies the same transformations as powerbi_cleaned.txt (Power Query M)
before loading. Demo-quality: no error handling, no logging, no tests.
A single transaction wraps the whole run — anything failing rolls all 9
loads back. Re-run after fixing.
"""
...
FILES = [
    ("Financial report YTD 2026.01.xlsx", "jan"),
    ("Financial report YTD 2026.02.xlsx", "feb"),
    ("Financial report YTD 2026.03.xlsx", "mar"),
]
YEAR = 2026
...
def main() -> None:
    ...
    with psycopg.connect(os.environ["PG_DSN"]) as conn, conn.cursor() as cur:
        for filename, month in FILES:
            for sheet_name, cfg in SHEET_CONFIGS.items():
                raw = pd.read_excel(base / filename, sheet_name=sheet_name, **cfg["read_kwargs"])
