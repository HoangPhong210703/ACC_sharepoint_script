...
Usage:
    # Read from local folder (default)
    python transform_financial_report.py

    # Read from SharePoint (requires: az login)
    python transform_financial_report.py --source sharepoint
"""
...
# ── SharePoint config (edit or set as env vars) ───────────────────────────────
import os
SP_HOSTNAME    = os.getenv("SP_HOSTNAME",     "savvycom.sharepoint.com")
SP_SITE_PATH   = os.getenv("SP_SITE_PATH",    "/sites/accsav")
SP_DRIVE_NAME  = os.getenv("SP_DRIVE_NAME",   "Shared Documents")
SP_FOLDER_PATH = os.getenv("SP_FOLDER_PATH",
                    "ACC-Savvycom/x. Tai lieu ke toan khac/Viet.Dinh/11. ACC Tool/Sample")
...
def _load_from_sharepoint() -> dict[str, bytes]:
    """
    Returns {filename: raw_bytes} for all matching Excel files in the SharePoint folder.
    Requires: az login (or az login --use-device-code on headless/Linux servers).
    """
    import requests
    ...
    site = requests.get(f"{base}/sites/{SP_HOSTNAME}:{SP_SITE_PATH}", headers=headers)
    site.raise_for_status()
    site_id = site.json()["id"]

    # Find the named drive (e.g. "Shared Documents")
    drives = requests.get(f"{base}/sites/{site_id}/drives", headers=headers)
    drives.raise_for_status()
    drive = next(
        (d for d in drives.json()["value"] if d["name"] == SP_DRIVE_NAME),
        None,
    )
    if drive is None:
        raise FileNotFoundError(f"Drive '{SP_DRIVE_NAME}' not found on site")
    drive_id = drive["id"]

    children = requests.get(
        f"{base}/drives/{drive_id}/root:/{SP_FOLDER_PATH}:/children",
        headers=headers,
    )
    children.raise_for_status()

    files = sorted(
        [f for f in children.json().get("value", [])
         if re.match(r"Financial report YTD \d{4}\.\d{2}\.xlsx", f["name"])],
        key=lambda f: f["name"],
    )
    if not files:
        raise FileNotFoundError(f"No matching files found in SharePoint folder: {SP_FOLDER_PATH}")

    print(f"SharePoint files: {[f['name'] for f in files]}")
    result = {}
    for f in files:
        # @microsoft.graph.downloadUrl is a pre-signed URL — no auth header needed
        resp = requests.get(f["@microsoft.graph.downloadUrl"])
        resp.raise_for_status()
        result[f["name"]] = resp.content
        print(f"  Downloaded: {f['name']} ({len(resp.content):,} bytes)")
    return result


def _bytes_to_path_like(name: str, raw: bytes):
    """Wrap raw bytes so transform functions can call pd.read_excel on it."""
    obj = io.BytesIO(raw)
    obj.name = name   # transform functions use .name to parse year/month
    return obj
...
def main():
    ...
    parser.add_argument("--source", choices=["local", "sharepoint"], default="local",
                        help="'local' reads from --input-dir; 'sharepoint' reads via az login")
    parser.add_argument("--input-dir", default=str(INPUT_DIR), help="Local folder (ignored when --source sharepoint)")
    ...
    if args.source == "sharepoint":
        print("Source: SharePoint")
        raw_files = _load_from_sharepoint()                          # {name: bytes}
        all_files = [_bytes_to_path_like(n, b) for n, b in raw_files.items()]
    else:
        input_dir = Path(args.input_dir)
        print(f"Source: local ({input_dir})")
